...
for (r in 1:225){ #Uses the reviews 1 to 7233 from PeerJ
  ...
  # download html
  html <- getURL(paste("https://peerj.com/articles/cs-", review_id, "/reviews/", sep=""), followlocation = TRUE, .mapUnicode = FALSE) #added .mapUnicode = FALSE otherwise reviews for 140 gave a bug
  ...
  html2 <- getURL(paste("https://peerj.com/articles/cs-", review_id,"/", sep=""), followlocation = TRUE)
  ...
  if (mark_count == 11){
    ...
    peerdoc <- append(peerdoc, "computer science")
  }
